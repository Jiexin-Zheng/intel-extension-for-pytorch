...
partition create_sdpa_graph_partition(
  ...
  if (enable_gqa) {
    q_sz_reshape = {
        batch_size, num_head_kv, num_head / num_head_kv, seq_len_q, head_dim};
    kv_sz_reshape = {batch_size, num_head_kv, 1, seq_len_k, head_dim};
    query_reshape_lt = {
        lt_id++, dtype, q_sz_reshape.value(), layout_type::strided};
    key_reshape_lt = {
        lt_id++, dtype, kv_sz_reshape.value(), layout_type::strided};
    query_reshape = {
        op_id++,
        op::kind::StaticReshape,
        {params.query},
        {query_reshape_lt.value()},
        "reshape1"};
  ...
  op matmul_qk{
      op_id++,
      op::kind::MatMul,
      {query_reshape_lt.value_or(params.query),
       key_reshape_lt.value_or(params.key)},
      {matmul_qk_out},
      "matmul_qk"};
  ...
  if (params.attn_mask.has_value()) {
    ...
    if (enable_gqa) {
      ...
      mask_reshape_lt = {
          lt_id++, dtype, mask_sz_reshape.value(), layout_type::strided};
      mask_reshape = {
          op_id++,
          op::kind::StaticReshape,
          {params.attn_mask.value()},
          {mask_reshape_lt.value()},
          "reshape3"};
  ...
  if (enable_gqa) {
    q_sz_reshape = {
        batch_size, num_head_kv, num_head / num_head_kv, seq_len_q, head_dim_v};
    output_reshape_lt = {
        lt_id++, dtype, q_sz_reshape.value(), layout_type::strided};
  }
...
void gpu_float_sdpa(
    ...
    float softmax_scale,
    const Tensor& output) {
  auto eng = torch_ipex::xpu::oneDNN::GpuEngineManager::Instance().get_engine(
  ...
  if (!cp_entry_ref.has_value()) {
    ...
    // partition cache should be disabled for GQA as it contains static reshape
    // ops
    if (!enable_gqa) {
...
_scaled_dot_product_fused_attention_overrideable_xpu(
  ...
  auto logsumexp = at::empty({}, opts.dtype(kFloat));
    ...
    if (is_causal) {
      attn_mask_fallback =
          at::ones_symint({seq_len_q, seq_len_kv}, opts.dtype(at::kBool))
              .tril();
      attn_mask_fallback =
          sdp::convert_boolean_attn_mask(attn_mask_fallback, query.dtype());
    } else {
      attn_mask_fallback = std::nullopt;
    }
  }
  at::AtenIpexTypeXPU::onednn::graph::gpu_float_sdpa(
      batch_size,
      seq_len_q,
      seq_len_kv,
      num_head,
  ...
  auto philox_seed = at::empty({}, dtype(kLong));
  auto philox_offset = at::empty({}, dtype(kLong));
  auto debug_attn_mask =
      at::empty({batch_size, num_head, seq_len_q, seq_len_kv}, dtype(kFloat));
...
_scaled_dot_product_efficient_attention(
  ...
  if (use_onednn_graph == nullptr || std::strcmp(use_onednn_graph, "0") == 0) {
    ...
    auto out = _scaled_dot_product_fused_attention_overrideable_xpu(
        query, key, value, attn_bias, dropout_p, is_causal, false, scale);
    return std::make_tuple(
        std::get<0>(out), std::get<1>(out), std::get<6>(out), std::get<7>(out));
  }
}
