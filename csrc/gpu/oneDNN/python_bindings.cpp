...
void sycl_free_func(
    ...
    void* event) {
  if (event) {
    sycl::event* sycl_deps_ptr = static_cast<sycl::event*>(event);
    sycl_deps_ptr->wait();
  }
...
void bind_engine(pybind11::module& m) {
  ...
  eng.def(pybind11::init([](engine::kind ekind, size_t id) {
    engine aengine;
#ifdef DNNL_WITH_SYCL
    if (ekind == engine::kind::gpu) {
      xpu::dpcpp::DeviceId dev_id = static_cast<xpu::dpcpp::DeviceId>(id);
      sycl::device& dev = xpu::dpcpp::dpcppGetRawDevice(dev_id);
      sycl::context& ctx = xpu::dpcpp::dpcppGetDeviceContext(dev_id);
      dnnl::graph::allocator alloc = dnnl::graph::sycl_interop::make_allocator(
          sycl_malloc_func, sycl_free_func);
      aengine = dnnl::graph::sycl_interop::make_engine_with_allocator(
          dev, ctx, alloc);
    } else if (ekind == engine::kind::cpu) {
      aengine = dnnl::engine(ekind, id);
    }
#else
    if (ekind == engine::kind::cpu) {
      aengine = dnnl::engine(ekind, id);
    }
#endif
    return aengine;
...
#define CASE(x)                        \
  case (logical_tensor::data_type::x): \
    return #x
...
void bind_op(pybind11::module& m) {
  ...
  pybind11::enum_<op::kind>(opr, "kind")
      ...
      .value("BatchNormTrainingBackward", op::kind::BatchNormTrainingBackward)
      ...
      .value("ConvolutionBackwardWeights", op::kind::ConvolutionBackwardWeights)
      .value("ConvTranspose", op::kind::ConvTranspose)
      .value("ConvTransposeBackwardData", op::kind::ConvTransposeBackwardData)
      .value(
...
void bind_tensor(pybind11::module& m) {
  ...
  t.def(pybind11::init([](logical_tensor& lt, engine& eng, pybind11::buffer b) {
...
void initOnednnGraphPythonBindings(pybind11::module& m) {
  ...
  auto onednn_graph = m.def_submodule("_onednn_graph");
  onednn_graph.doc() = R"pbdoc(
        oneDNN Graph API Python binding
        -------------------------------
        .. currentmodule:: onednn_graph
        .. autosummary::
        :toctree: _generate
    )pbdoc";
  bind_status(onednn_graph);
  bind_graph(onednn_graph);
  bind_logical_tensor(onednn_graph);
  bind_engine(onednn_graph);
  bind_op(onednn_graph);
  bind_tensor(onednn_graph);
  bind_partition(onednn_graph);
  bind_cpartition(onednn_graph);
  bind_stream(onednn_graph);
}
